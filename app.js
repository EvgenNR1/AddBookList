...
const taskInput1 = document.querySelector('#task1');
...
function loadEventListeners(){
    //gets tasks from local storage on load
...
function getTasks(){
    ...
    tasks.forEach(function(task){ //or lfie function task=>  or function(task){
...
function addTask(event){
    ...
    li.appendChild(document.createTextNode(taskInput.value+" - "+taskInput1.value));
    ...
    storeInLocalStorage(taskInput.value+" - "+taskInput1.value);

    taskInput.value=''; // does not work
    taskInput1.value='';

    event.preventDefault();

    // storeInLocalStorage(taskInput1.value);
    // taskInput1.value=''; // does not work
    // event.preventDefault();
}
